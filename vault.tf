...
# Boundary polices for OIDC Read access
...
# Engineering user and group for Boundary
...
resource "vault_identity_oidc_scope" "users" {
  name        = "user"
  template    = "{ \"username\": {{identity.entity.name}}, \"contact\": { \"email\": {{identity.entity.metadata.email}}, \"phone_number\": {{identity.entity.metadata.phone_number}} } }"
...
# Read in Boundary auth and secret
data "vault_kv_secret" "boundary_auth" {
  path = "kv/data/boundary-auth"
}

output "secret_01" {
    value = data.vault_kv_secret.boundary_auth.data
    sensitive = true
}